...
Установите Postman или любой другой инструмент для выполнения http-запросов и протестируйте эти запросы. Так же вам доступен веб-доступ, вы можете проверить как отрабатывают запросы во фронтенд-приложении через devtools. Чтобы получить полный адрес запроса, зайдите в веб-доступ и скопируйте урл страницы. Соедините этот урл с адресом запроса. Например, так может выглядеть адрес получения списка товаров: https://web-testing-phase-testing-system-4643087.evaluator6-5.hexlet.io/api/products — к урл добавлено `/api/products`.
...
В запросах, где требуется авторизация, в заголовках запроса должен идти специальный токен — он нужен, чтобы сервер правильно определил пользователя, от которого идет запрос. Для этого в Postman, на вкладке Authorization укажите `Bearer Token`. Перед этим вы должны получить этот токен отправив запрос на создание пользователя или запрос аутентификации.
...
Юрий Ворошилов 29 января 2024 

Задание интересное, занятное. Хорошая работа с базовыми запросами. Больше всего пришлось повозиться с токеном) не с первого раза и не со второго получилось)

* * *

Александр Бобриков 01 октября 2023 

очень необычное испытание, мне понравилось) спасибо
