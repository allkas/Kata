...
В общих чертах процесс отображения страницы выглядит так:
  ...
  1. Браузер узнает адрес страницы с помощью DNS, а затем делает запрос на сервер: 
...
Сервер присылает в ответ HTML-код.
  ...
  2. Браузер получает HTML-код, анализирует его и формирует внутреннюю структуру, называемую **DOM-дерево**.

  3. После этого браузер использует это дерево для физического рендеринга страницы, которую мы рассматриваем:
...
#### Самостоятельная работа
  ...
  2. Используйте объект `document` для замены тела документа на следующий фрагмент:
