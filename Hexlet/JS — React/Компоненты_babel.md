...
Нужно понимать, что JSX — расширение языка, а значит это именно _код_ , а не `html`. А раз JSX транслируется в код, то, следовательно, вы могли бы сразу писать этот код. Верно? Верно, но не совсем: 
