...
В таком случае лучше вынести построение пути в отдельную функцию, а заодно воспользоваться правильным способом склеивания путей: 
    ...
    const html = readFileSync('withLinks.html');
    const json = readFileSync('somethingElse.json');
...
Не забудьте после изменения кода загрузить изменения в репозиторий Github.Решение[Репозиторий с результатом](https://github.com/hexlet-components/hexlet-js-testing)

* * *

#### Дополнительные материалы

  1. Что такое __dirname в JavaScript
