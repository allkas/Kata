...
Мы можем модифицировать код так, что метод `toString()` переместится в _HTMLElement_. И единственная вещь, которая останется за подклассами – имя тега: 
...
Создадим у _HTMLElement_ два подкласса: один _HTMLSingleElement_ и _HTMLPairElement_. Теперь классы конкретных тегов должны наследоваться от одного из указанных классов. В каждом из этих классов будет своя реализация метода `toString()`. 
